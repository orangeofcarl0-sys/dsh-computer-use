...
export async function screenObserve(ctx, args, cfg, exec) {
  ...
  const driverOrigin = target.bounds ? { x: target.bounds.x, y: target.bounds.y } : null
  ...
  if (IS_WINDOWS && cfg.passwordScan !== 'off' && elements.length > 0) {
    ...
    if (scan) {
      const origins = [driverOrigin, { x: scan.window.left, y: scan.window.top }].filter(Boolean)
      const marked = markElementsFromScan(elements, scan, { origins })
...
export async function screenZoom(ctx, args, cfg, exec) {
  ...
  const scale = +(cropW / (image.width || cropW)).toFixed(4)
  return {
    ok: true,
    result: `区域截图完成（${image.width}×${image.height}px JPEG，对应整窗区域 (${cx1},${cy1}) ${cropW}×${cropH}，缩放 ×${scale}）。`
      + `定位环用法：本图内目标坐标 × ${scale} + (${cx1},${cy1}) = 整窗截图像素坐标，可直接 computer_click(x=,y=)。`,
