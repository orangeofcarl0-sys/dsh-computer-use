...
/**
 * 权限模式钩子（T1 信息获取全权限）：
 *   - full-access / unrestricted：窗口级观测失败 → 桌面级采集兜底（视觉仅读）；
 *     AX 树为空 → 标注 visualOnly；观测结果附驱动权限面。
 *   - 动作侧护栏（TTL / 批准 / 密码框）不受本模式影响（unrestricted 例外见 guard.js）。
 */
function observeFull(cfg) {
  return cfg.permissionMode === 'full-access' || cfg.permissionMode === 'unrestricted'
}
...
/**
 * 桌面级兜底观测（full-access/unrestricted）：get_desktop_state 全屏视觉采集。
...
export async function screenObserve(ctx, args, cfg, exec) {
  ...
  const access = observeFull(cfg) ? await driverAccess() : null
  ...
  const visualOnly = degraded && observeFull(cfg)
  const header = [
...
export async function screenZoom(ctx, args, cfg, exec) {
  ...
  try {
    ...
    if (!observeFull(cfg) || !size?.screenshot_png_b64) throw err
    try {
      const ref = await attachments.saveImage({
