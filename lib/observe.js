...
/** 快照失效/消费/切换窗口时由动作侧调用，避免陈旧哈希被复用。 */
export function dedupInvalidate(pid, windowId) {
  for (const k of [...DEDUP_STATE.keys()]) {
    if (k.startsWith(`${pid}:${windowId}:`)) DEDUP_STATE.delete(k)
  }
}
...
export async function screenObserve(ctx, args, cfg, exec) {
  ...
  // 4.5 结构性密码标记（Windows UIA sidecar；失败/skipped 静默降级启发式，PLAN-credential-guard 第二期）
  let scan = null
  let scanNote = ''
  if (IS_WINDOWS && cfg.passwordScan !== 'off' && elements.length > 0) {
    scan = await scanPasswords({ hwnd: target.window_id, timeoutMs: 5000 })
    if (scan) {
      const marked = markElementsFromScan(elements, scan)
      if (marked > 0) scanNote = `（结构密码标记 ${marked} 项：IsPassword/ES_PASSWORD 实测）`
    } else {
      scanNote = '（结构密码扫描不可用，凭据保护运行于启发式）'
    }
  }
  ...
  // 判定在元素映射完成后做（哈希用换算后的坐标，保证与模型所见一致）。
  ...
  return {
    ...
    // 因此这里在**输出时**换算到点击空间：截图、元素坐标、点击三者同一个空间，模型"所见即所点"。
...
export async function screenZoom(ctx, args, cfg, exec) {
  ...
  // pid 缺省时按 window_id 从窗口表解析（引擎 zoom 需要 pid）；顺带取 bounds 作为屏幕原点
  let pid = args.pid
  let zoomTarget = null
  {
    const winList = await cuaCall('list_windows', { on_screen_only: true })
  ...
  // 换算比例以**驱动返回的图尺寸**为准（实测驱动会对裁剪做恒定 1.2× 上采样：
