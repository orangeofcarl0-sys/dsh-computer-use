...
import { UNVERIFIED_NOTE, receipt, refusalReceipt, settleAction } from './receipt.js'
...
async function humanAction(label, tool, args, cfg, extra = {}) {
  ...
  const refusalNote = /element_not_visible/i.test(engineRefusal(value) || '')
    ...
    : ''
  return settleAction(tool, value, { label, note, gateNote: gate.note, refusalNote, verbose: cfg.verboseReceipts === true })
}
...
export async function typeText(args, cfg) {
  ...
  return settleAction('type_text', vRaw, { label: '输入', note, gateNote: gate.note, verbose: cfg.verboseReceipts === true })
}
...
export async function key(args, cfg) {
  ...
  if (refusal) {
    ...
    return refusalReceipt('按键', refusal, deliveryNote)
  }
  return {
    ok: true,
    result: receipt('按键', value, {
      key: String(args.key),
      extraNote: channelNote + deliveryNote + (isUnverified(value) ? UNVERIFIED_NOTE : '') + gate.note,
      unverified: isUnverified(value),
      verbose: cfg.verboseReceipts === true,
    }),
  }
}
...
export async function scroll(args, cfg) {
  ...
  return settleAction('scroll', vRaw, {
    label: '滚动',
    key: dir,
    note,
    gateNote: freshnessGate(cfg).note,
    verbose: cfg.verboseReceipts === true,
  })
}
