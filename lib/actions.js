...
/**
 * 紧凑回执（PLAN-context-budget C，拍板"保守两步"）：正常路径只回一行要点，
 * 原始 JSON 明细仅在拒绝/不可验证/verboseReceipts=true 时附上——回执会随历史每步重发，
 * 明细常驻是纯浪费（实测 click 299 字符里 200+ 是 JSON）。
 */
function receipt(label, value, { key = '', note = '', extraNote = '', verbose = false, unverified = false } = {}) {
...
async function humanAction(label, tool, args, cfg, extra = {}) {
  ...
  return {
    ok: true,
    result: receipt(label, value, { note, extraNote: (isUnverified(value) ? UNVERIFIED_NOTE : '') + gate.note, unverified: isUnverified(value), verbose: cfg.verboseReceipts === true }),
  }
}
...
export async function typeText(args, cfg) {
  ...
  const detail = typeof value === 'string' ? value : JSON.stringify(value)
  return { ok: true, result: receipt('输入', value, { note, extraNote: (isUnverified(value) ? UNVERIFIED_NOTE : '') + gate.note, unverified: isUnverified(value), verbose: cfg.verboseReceipts === true }) }
}
...
export async function key(args, cfg) {
  ...
  return {
    ok: true,
    result: receipt('按键', value, { key: String(args.key), extraNote: channelNote + deliveryNote + (isUnverified(value) ? UNVERIFIED_NOTE : '') + gate.note, unverified: isUnverified(value), verbose: cfg.verboseReceipts === true }),
  }
}
...
export async function scroll(args, cfg) {
  ...
  if (!refusal) {
    markConsumed('scroll')
    if (isUnverified(value)) markSuspect('scroll')
  }
  if (refusal) return { ok: false, result: `滚动被引擎拒绝：${refusal}${note}` }
  const detail = typeof value === 'string' ? value : JSON.stringify(value)
...
export async function drag(args, cfg) {
  ...
  if (!refusal) {
    markConsumed('drag')
    if (isUnverified(value)) markSuspect('drag')
  }
  if (refusal) return { ok: false, result: `拖拽被引擎拒绝：${refusal}${note}` }
  const detail = typeof value === 'string' ? value : JSON.stringify(value)
...
/** app_list —— 列出应用（紧凑格式：一行一应用，超限截断） */
export async function listApps() {
  const value = await cuaCall('list_apps')
  const apps = (value.apps || []).filter((a) => a.running)
  const CAP = 20
  const shown = apps.slice(0, CAP)
  const lines = shown.map((a) => {
    const win = a.windows && a.windows.length > 0 ? ` ×${a.windows.length}` : ''
    return `${a.name}${win} pid=${a.pid}${a.active ? ' ★' : ''}`
  })
  const more = apps.length > CAP ? `\n（其余 ${apps.length - CAP} 个未列出；可直接用 window=<pid|标题子串> 指定目标）` : ''
  const result = `运行中应用 ${apps.length} 个：\n${lines.join('\n')}${more}`
