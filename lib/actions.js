...
async function runAction(label, tool, args, cfg, extra = {}) {
  ...
  return {
    ok: true,
    result: `${label} 完成：${detail}${note}${isUnverified(value) ? UNVERIFIED_NOTE : ''}`,
  }
}
...
async function humanAction(label, args, cfg, extra = {}) {
  ...
  return {
    ok: true,
    result: `${label} 完成（虚拟光标滑行+点击）：${detail}${note}${isUnverified(value) ? UNVERIFIED_NOTE : ''}`,
  }
}
...
export async function typeText(args, cfg) {
  ...
  const detail = typeof value === 'string' ? value : JSON.stringify(value)
  return { ok: true, result: `输入完成：${detail}${note}${isUnverified(value) ? UNVERIFIED_NOTE : ''}` }
}
...
export async function key(args, cfg) {
  ...
  const channelNote = isCombo
    ...
    : ''
  const refusal = engineRefusal(value)
  if (refusal) return { ok: false, result: `按键被引擎拒绝：${refusal}${deliveryNote}` }
  const detail = typeof value === 'string' ? value : JSON.stringify(value)
  return {
    ok: true,
    result: `按键完成 (${String(args.key)})：${detail}${channelNote}${deliveryNote}${isUnverified(value) ? UNVERIFIED_NOTE : ''}`,
  }
}
